...
// Import adjustment: @/directives/permission/index → @/directives/permission/index (app-platform local)
...
const importFormData = reactive<{ files: UploadUserFile[] }>({ files: [] });
...
function handleModify(field: string, value: boolean | string | number, row: Record<string, any>) {
  if (props.contentConfig.modifyAction) {
    props.contentConfig.modifyAction({ [pk]: row[pk], field, value });
  } else {
